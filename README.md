<div align="center">
  <h1>Krishna Parashar</h1>
  <p><i>Computer Science & Engineering Undergraduate @ Indian Institute of Technology Guwahati</i></p>
  
  <p>
    <a href="https://www.linkedin.com/in/krishna-parashar-639706317/">LinkedIn</a> •
    <a href="https://codeforces.com/profile/Krishna_1107">Codeforces (Expert)</a> •
    <a href="mailto:k.parashar@iitg.ac.in">Email</a>
  </p>
</div>

---

## ⚡ Current Focus

* **Studying:** Pursuing my B.Tech at IIT Guwahati with a current CGPA of 9.74. Secured an All India Rank of 607 in JEE Advanced.
* **Building:** A High-Performance 5-Stage Pipelined Processor in Verilog. Recently redesigned the synchronous hazard control unit to eliminate forwarding from the write-back stage, and integrated a Branch Target Buffer (BTB) with an LRU L1 cache.
* **Leading:** Serving as the Web Development Module Coordinator for the IITG Coding Club. Moderated the Jane Street-sponsored IIWCPC contest.
* **Competing:** Winning team member at Atlassian Fast Track 2026 and Global Rank 730 among 12,000+ participants in IICPC Codefest 2026.

<br>

## ⌘ Technical Arsenal

<a href="https://skillicons.dev">
  <img src="https://skillicons.dev/icons?i=c,cpp,java,js,ts,react,nextjs,nodejs,express,tailwind,mongodb,mysql,supabase,linux,git" alt="My Skills" />
</a>
<br>

## ▤ Featured Projects

### [↗ NoteVell: AI-Assisted Notetaking](https://github.com/Krishna-1107/NoteVell.git)
> **Stack:** Next.js, TypeScript, Tailwind CSS, Supabase

A modern, full-stack web application incorporating the Gemini API so users can interact with their data through natural language queries. 

### [↗ IITG HAB Portal](https://github.com/HABIITG/iitgHABapp)
> **Stack:** React.js, MongoDB

A dashboard developed for the IITG Hostel Affairs Board to parse and manage student mess allocation data.

### [↗ High-Performance 5-Stage Pipelined Processor](https://github.com/GRP-3-RISCV/HighPerformance_5stage_Pipeline_Processor)
> **Stack:** Verilog

Hardware lab project simulating core pipeline stages, featuring a robust synchronous hazard control unit, BTB, and L1 caching.

<br>

## ✆ Connect

[↗ LinkedIn](https://www.linkedin.com/in/krishna-parashar-639706317/) &nbsp;&nbsp;|&nbsp;&nbsp; [↗ Codeforces](https://codeforces.com/profile/Krishna_1107) &nbsp;&nbsp;|&nbsp;&nbsp; [↗ Email](mailto:k.parashar@iitg.ac.in)

<br>

---
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Krishna-1107&show_icons=true&theme=transparent&hide_border=true&title_color=black&text_color=black&icon_color=black" alt="Krishna's GitHub Stats" />
</p>
